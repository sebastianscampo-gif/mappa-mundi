...
//   #/archive/_/q=foo          → Search results
//   #/map/ID
...
window.addEventListener("mappa:account-changed", () => {
  ...
  if (page === "account" || page === "library") renderRoute();
});
...
function renderHome() {
  ...
  // Recent strip — random sample
  const seedSample = MAPS.filter(m => m.renderable && m.description).slice(0, 30);
  const recent = [];
  while (recent.length < 5 && seedSample.length) {
    const idx = Math.floor(Math.random() * seedSample.length);
    recent.push(seedSample.splice(idx, 1)[0]);
  }
...
function renderCategoryGrid(root) {
  ...
  $$('[data-era]').forEach(el => el.addEventListener("click", (e) => {
    ...
    archiveState.currentCategory = "_all";
    renderArchive();
  }));
...
function renderCategoryDetail(root, catKey) {
  const all = catKey === "_all" ? MAPS : (MAPS_BY_CATEGORY[catKey] || []);
  const meta = catKey === "_all" ? { display:"All maps", subtitle:"Every record in the archive.", description:"The complete archive across all 16 categories." } : categoryMeta(catKey);
  const cover = catKey === "_all" ? MAPS.find(m => m.renderable) : categoryCoverImage(catKey);
  const filtered = filterMaps({ category: catKey === "_all" ? null : catKey });
...
function filterMaps(opts = {}) {
  let list = opts.category ? (MAPS_BY_CATEGORY[opts.category] || []) :
             (archiveState.currentCategory && archiveState.currentCategory !== "_all" ? MAPS_BY_CATEGORY[archiveState.currentCategory] : MAPS);
  list = list.slice();

  if ((opts.search ?? archiveState.search)) {
    const q = (opts.search ?? archiveState.search).toLowerCase();
    list = list.filter(m =>
      (m.title + " " + (m.original_title||"") + " " + (m.region||"") + " " + (m.country||"") +
       " " + (m.author||"") + " " + (m.description||"") + " " + (m.historical_context||"") +
       " " + (m.tags||[]).join(" ") + " " + (m.institution||"") + " " + (m.continent||"") +
       " " + (m.language||"") + " " + categoryDisplay(m.category)).toLowerCase().includes(q)
    );
  }
  if (archiveState.era !== "all") {
...
function wireResultControls() {
  ...
  $("#cat-search input")?.addEventListener("input", (e) => {
    archiveState.search = e.target.value.trim();
    archiveState.page = 1;
    // re-render just the result panel for speed
    renderArchive();
  });
  // Clear all
        ...
        <div class="detail-viewer">
          <div class="viewer-toolbar">
            ...
            <div class="row" style="gap:6px">
              ...
              <button class="icon-btn" title="Save">${icons.bookmark}</button>
          ...
          <div class="viewer-actions">
            <button class="btn btn-primary" data-action="save">${icons.bookmark} Save to library</button>
...
function renderLibrary() {
  const rich = MAPS.filter(m => m.renderable && m.description);
  const saved = rich.slice(0, 4);
  const recent = rich.slice(4, 9);
  ...
  $("#library-notes").innerHTML = noteTargets.map((m, i) => `
    <div class="note">
      <div class="note-anchor">
        <span class="meta">on</span>
        <a href="#/map/${m.id}" class="link-underline">${shortTitle(m.title, 60)}</a>
      </div>
      <p>${noteTexts[i]}</p>
      <span class="meta">${["12 March 2026", "04 March 2026", "22 February 2026"][i]}</span>
    </div>
  `).join("");
}
